function bid_ask() {
    ...
    for (let i = 0; i < table[0].children[1].childNodes.length; i++) {
        ...
        if (strike_price !== null && strike_price.childNodes[0].data === selected_strike) {
            let bid = document.querySelector("body > form > table.call-table > tbody > tr:nth-child(" + i + ") > td:nth-child(2)").childNodes[0].data
            let ask = document.querySelector("body > form > table.call-table > tbody > tr:nth-child(" + i + ") > td:nth-child(3)").childNodes[0].data
            result = {
